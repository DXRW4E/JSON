...
Sorry guys but my English is that of Google translate ehhh, what can I say I needed it for personal use and I started writing this code in Android, believe it or not but I wrote all the JSON.c in Samsung A14 using the Cxxdroid (which also includes curl).

Guys to be honest, I don't even have free time to follow or maintain or update the project, anyway I thought to publish it

what can I say, everything you need there is the struct SJSON and the JSONParse() and JSONFree(), Once you have a good structure, then do whatever you want (really you can do everything), and how you want so all other functions, they are just to give an example, as it is easy (with few lines) to do everything, as you will see JSONOpenKeyEx() also does everything in a perfect simple and fast way.

guys there is always a bug ehhh, but the JSONParse() and JSONStringify() and JSONFree() is written in the simplest way possible, to be simple, fast (and it is really the fastest comparing to all others, at least from the quick tests performed here by me) and efficient, so I believe they will never have to cause problems.

Regarding all the other function, even if they are Well Done, they serve more as an example, since the JSON structure allows you to do everything there, so you can go even beyond them.

as already said there is plenty of choice there, however I chose MS Registry\File Function mod, so JSON function works\like Registry function Open Handle & Close Handle ect ect you can also use LockHandle ect ect, I think it's the easiest fastest and most familiar way,
Guys in the end it's just JSON Read\Write, it's not an Operating System ehhh, I mean there it's all simple, and just one edit file ehhh

Guys never say never, but logically\nomraly everything should always be ok there, having said this since [in C++ If a capacity change takes place, all iterators and references, including the past-the-end iterator, are invalidated.](https://en.cppreference.com/w/cpp/string/basic_string/reserve) this doesn't happen using JSON.c, as I already said it's just a json file, you don't even need to open\work with 100 handles, in any case everything should be ok there.

guys I don't know, but normally, you don't even need to delete key there, since once finished you use JSONStringify(), so I recommend using #define _JSON_DELETE_ON_CLOSE, anyway it's all fast there, I don't know, see for yourself what to do there

Finally, I tested it on Android Smartphone A14, and old NoteBook win Windows 10 in VS2020, the code is there, if you want you can try it (and do the tests with files.json 2/3/4 MB)

Guys, to not complicate it unnecessarily, it's all simple there, is everything like JavaScript
So (open\edit\create) json file\data using JSONParse() and use JSONStringify() and that's it, for more se examples


Guys, you only need to add `JSON.h` and `JSON.c` to your projects source and that's it
...
int main() {
	HJSON hJSON = NULL, hJSKey = NULL, hJSElement = NULL; LPWSTR lpszJSON = NULL, lpStrVal = NULL; LPCWSTR lpEnd = NULL; DWORD dwLastError = 0, cchStr = 0;
	...
	if (dwLastError) { wprintf(L"1. JSONOpenKey ERROR %d \n", dwLastError); }
		...
		if (JSONPtr(hJSKey)->IsContainer) { wprintf(L"hJSKey - MemberCount %d  - MemberBuffer %d \n", JSONContainerLength(hJSKey), JSONContainerSize(hJSKey)); }
			...
			lpStrVal = JSONGetStringValue(hJSKey);
        	if (!lpStrVal) { wprintf(L"2. JSONGetStringValue ERROR %d \n", GetLastError()); }
        	else { wprintf(L"3. lpStrVal %ls\n", lpStrVal); MemFree(lpStrVal); lpStrVal = NULL; }
			// or
			if (JSONPtr(hJSKey)->IsEscapeStr) { lpStrVal = JSONUnEscapeStrEx(hJSKey); wprintf(L"3. lpStrVal %ls\n", lpStrVal); MemFree(lpStrVal); lpStrVal = NULL; }
