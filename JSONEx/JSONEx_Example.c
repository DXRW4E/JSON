...
#else
#include "/storage/emulated/0/JSON/JSONEx.c"
...
int main() {
	...
	//
	LPWSTR lpszJSON = NULL, lpStrVal = NULL; LPCWSTR lpEnd = NULL; DWORD cchStr = 0, dwLastError = 0, dwDisposition = 0, dwIndex = 0;
	HJSON hJSON = NULL, hJSONEx = NULL, hJSCopy = NULL, hJSNew = NULL, hJSONMrg = NULL, hJSKeyEx = NULL, hJSElement = NULL, hJSEnumKey = NULL, hJSKey = NULL, hJSNewKey = NULL, hJSArray = NULL; LPWSTR lpStrfy = NULL;
/* 
...
*/
	...
	wprintf(L"\nTest JSONMerge\n");
 	hJSONEx = JSONParse(wsJSONStrEx, 0, &lpEnd); //	lpszJSONEx = &wsJSONStrEx[0]; cchStrEx = StrLen(lpszJSONEx); hJSONEx = JSONParse(lpszJSONEx, cchStrEx, &lpEnd); wprintf(L"1. StrLen %d \n", cchStrEx);
	if (!hJSONEx) { wprintf(L"1. JSONParse ERROR %d %ls\n", GetLastError(), lpEnd); JSONFree(hJSON); return 0; }
	hJSONMrg = JSONCopyTreeEx(hJSON);
	if (!hJSONMrg) { wprintf(L"2. JSONCopyTree ERROR %d \n", GetLastError()); }
	else {
		dwLastError = JSONMerge(hJSONMrg, hJSONEx, 0);
		if (dwLastError) { wprintf(L"3. JSONMerge ERROR %d \n", dwLastError); }
		else {
			wprintf(L"1. isHeader %d \n", JSONPtr(hJSONMrg)->IsHeader);
			wprintf(L"2. isObject %d \n", JSONPtr(hJSONMrg)->IsObject);
			wprintf(L"3. dwMemCount %d \n", JSONContainerLength(hJSONMrg));
			wprintf(L"4. JSONGetStringifySize %d\n", JSONGetStringifySize(hJSONMrg));
			lpStrfy = JSONStringify(hJSONMrg);
			if (!lpStrfy) { wprintf(L"5. JSONStringify ERROR %d \n", GetLastError()); }
			else { wprintf(L"5. lpStrfy %ls\n", lpStrfy); MemFree(lpStrfy); lpStrfy = NULL; }
		}
		JSONClose(hJSONMrg); hJSONMrg = NULL;
	}
	hJSONMrg = JSONCopyTreeEx(hJSON);
	if (!hJSONMrg) { wprintf(L"4. JSONCopyTree ERROR %d \n", GetLastError()); }
	else {
		dwLastError = JSONMerge(hJSONMrg, hJSONEx, JSON_REPLACEONLY);
	...
	hJSONMrg = JSONCopyTreeEx(hJSON);
	if (!hJSONMrg) { wprintf(L"6. JSONCopyTree ERROR %d \n", GetLastError()); }
	else {
		dwLastError = JSONMerge(hJSONMrg, hJSONEx, JSON_NOOVERWRITE);
	...
	JSONClose(hJSONEx); hJSONEx = NULL;
	////JSONFree(hJSON); hJSON = NULL;
